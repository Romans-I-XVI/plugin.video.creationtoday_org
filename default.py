...
def MAIN():
	...
	addDir('Creation Minute', 'http://www.creationminute.com',2,'')
...
def Creation_Minute(url):
	link = getUrl(url)
	match=re.compile('<a href="http://www.youtube.+?embed/(.+?)rel').findall(link)
	title=re.compile('<h5 style="color:white; margin-bottom:0px;">(.+?)</h5>').findall(link)
	mylist=zip((match),(title))
	for match,title in mylist:
		match=match[:-1]
		xbmc.log('test123'+match)
		addon.add_video_item({'url': 'http://www.youtube.com/watch?v=' + match},{'title': title})
...
def GETSOURCE(url,name,iconimage):
	link = getUrl(url)
	match_vimeo=re.compile('<iframe src="http://player.vimeo.com/video/(.+?)title').findall(link)
	match_youtube=re.compile('src=".+?www.youtube.+?embed/(.+?)rel').findall(link)
	try:
		match_vimeo=match_vimeo[0]
		site=0
	except:
		match_youtube=match_youtube[0]
		match_youtube=match_youtube[:-1]
		site=1
	if site==0:
		match=addon.resolve_item({'url': 'http://www.vimeo.com/' + match_vimeo})
	if site==1:
		match=addon.resolve_item({'url': 'http://www.youtube.com/watch?v=' + match_youtube})
